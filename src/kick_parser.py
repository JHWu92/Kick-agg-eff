...
import json
from url_helper import parse_url_qs

class BackerCity:
    
    def __init__(self, city, country, cnt, woeid):
        self.city = city
    ...
    def to_dict(self):
        return {'city': self.city, 'country': self.country, 'cnt': self.cnt, 'woeid':self.woeid}
...
class BackerCountry:
    ...
    def __init__(self, country, cnt, woeid):
        self.country = country
        self.cnt = cnt
        self.woeid = woeid
    
    def to_dict(self):
        return {'country': self.country, 'cnt': self.cnt,  'woeid':self.woeid}
...
def parse_commnunity_page(soup, res, debug=False):
    ...
    cities = soup.find('div', class_='community-section__locations_cities')
    if cities:
        cities = cities.find_all('div', class_='location-list__item js-location-item')
        backer_cities = []
        for c in cities:
            ...
            woeid = parse_url_qs(c.find('a')['href'])['woe_id'][0]
            bc = BackerCity(city=cityname, country=countryname, cnt=cnt, woeid=woeid)
            backer_cities.append(bc.to_dict())
        res['backer_cities'] = json.dumps(backer_cities)
    
    countries = soup.find('div', class_='community-section__locations_countries')
    if countries:
        countries = countries.find_all('div', class_='location-list__item js-location-item')
        backer_countries = []
        for c in countries:
            countryname = c.find('div', 'primary-text js-location-primary-text').text.strip()
            cnt = c.find('div', 'tertiary-text js-location-tertiary-text').text.replace('backers', '').replace('backer', '').strip()
            woeid = parse_url_qs(c.find('a')['href'])['woe_id'][0]
            bc = BackerCountry(country=countryname, cnt=cnt, woeid=woeid)
